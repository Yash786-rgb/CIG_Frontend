...
import * as data from "./teamMember";
...
class About extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      lecture: {
        heading: "Mission",
        sub: "Building a stronger connection between Academia & Industry via valuable collaborations",
        link: "",
      },
      workshop: {
        heading: "Vision",
  ...
  render() {
    return (
      <div>
        <div id="about_heading">Corporate Interaction Group IIT Roorkee</div>
        <div id="about_sub_heading">
          Corporate Interaction Group(CIG) is a student body of IITR founded on
          1st October 2015 that works for creating and nurturing excellent
          relationships with Industries catering to technological and research
          needs of Industry and walking together towards innovating our society
          that is mutually beneficial to both IIT Roorkee and the Industries
        </div>
        ...
        <div className="row">
          {data.adminBody.length !== 0 &&
            data.adminBody.map((adm, index) => <Card id={index} admin={adm} />)}
        </div>

        <div id="std">Student body</div>

        <div className="row">
          {data.studentBody.length !== 0 &&
            data.studentBody.map((adm, index) => (
